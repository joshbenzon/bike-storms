...
# df_init = pd.read_parquet('taxi_orig/yellow_tripdata_2021-01.parquet')
...
df_merged = df_merged[['date', 'duration', 'trip_distance']]
...
df_average = df_merged.groupby('date').agg({'duration': 'mean', 'trip_distance': 'mean'}).rename(columns={'duration': 'avg_duration', 'trip_distance': 'avg_distance (km)'})
